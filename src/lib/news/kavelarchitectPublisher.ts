...
interface PublishKavelarchitectInput {
  id: string
  title: string
  summary: string | null
  body: string
...
export interface PublishKavelarchitectResult {
  slug: string
  url: string
  filePath: string
}

const DEFAULT_REPO_PATH = process.platform === 'win32' ? 'E:\\Funda Wordpress' : '/mnt/e/Funda Wordpress'
const DEFAULT_FEATURED_IMAGE = '/hero-bg.jpg'

export async function publishToKavelarchitectNieuws(input: PublishKavelarchitectInput): Promise<PublishKavelarchitectResult> {
  const repoRoot = process.env.KAVELARCHITECT_REPO_PATH?.trim() || DEFAULT_REPO_PATH
  ...
  return {
    slug,
    url: `https://kavelarchitect.nl/nieuws/${slug}`,
...
async function ensureRepositoryReady(indexFilePath: string) {
  try {
    ...
    throw new Error(`KavelArchitect repo niet gevonden op verwachte locatie: ${indexFilePath}`)
  }
}
...
function buildArticleFile({
  ...
  return [
    ...
    '  },',
    '  seo: {',
    `    title: '${escapedTitle} | KavelArchitect nieuws',`,
...
function mapCategory(sourceType: string): 'markt' | 'regelgeving' | 'project' | 'bedrijf' {
  ...
  if (value === 'manual') return 'bedrijf'
...
function buildExcerpt(summary: string | null, body: string) {
  ...
  return truncate(base || 'Actueel nieuwsbericht van KavelArchitect.', 220)
}
