...
export async function getOverviewData(): Promise<OverviewData> {
  ...
  const [workspacesRes, pagesRes, snapshotsRes, clustersRes, mentionsRes, nieuwsRes] = await Promise.all([
    ...
    supabase.from('gsc_snapshots').select('pillar_page_id, clicks, snapshot_date').gte('snapshot_date', sinceDate),
  ...
  const stats = new Map<string, WorkspaceStat>(
    workspaces.map((workspace) => [workspace.id, { ...workspace, pages: 0, pagesWithSchema: 0, clicks30d: 0, mentions30d: 0, pendingNieuws: 0 }])
  )
